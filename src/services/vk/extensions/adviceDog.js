...
export async function getPopular () {
    ...
    return [...function * () {
        ...
        for (let [id, vector] of cache)
            if (Math.min(...list
                    .map(id => cache.get(id))
                    .map(findVectorsAngleSin.bind(this, vector))
                    .map((distance, idx, arr) => distance * arr.length / (idx + 1))) > 0.3)
                yield list.push(id) && id
    }()]
}
...
//noinspection JSUnusedGlobalSymbols
export async function getClosest () {/*todo*/
    const cache = await vectorCache
    const userGroups = new Set((await this.getFavorites()).filter(a => a))
    ...
    const sourceItems = new Set(
        (await Promise.all(function * () {
            for (let id of userGroups)
                yield cache.has(id)
                    ? id
                    : this.getGroupInfo(id, ['members_count'])
    ...
    await Promise.all(function * () {
        for (let sourceItem of sourceItems)
            yield this.getGroupUsers(sourceItem)
    ...
    for (let [user, userSourceItems] of user2sourceItem)
        if (userSourceItems.size > 2) {
            user2sourceItemFiltered.set(user, userSourceItems)
            for (let sourceItem of userSourceItems)
    ...
    await Promise.all(function * () {
        for (let [item, users] of targetItem2user)
            yield this.getGroupInfo(item, ['members_count'])
