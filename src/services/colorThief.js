...
export function imageLoadPromise (src) {
    return fetch(src)
        .then(response => response.blob())
        .then(blob => Object.assign(document.createElement('img'), {src: URL.createObjectURL(blob)}))
...
export function colorThief (image, {step = 10, allowWhite = false, expectTransparent = false, colorCount = 5} = {}) {
    ...
    try {
        var {data: pixels} = context.getImageData(0, 0, image.width, image.height);
    } catch (e) {
        ...
        for (var offset = 0; offset < offsetMaxValue; offset += offsetStep)
            if (pixels[offset + 3] >= 125)
                if (pixels[offset] <= colorThreshold ||
                    ...
                    pixelArray.push(pixels.slice(offset, offset + 3));
    }
        : function () {
        for (var offset = 0; offset < offsetMaxValue; offset += offsetStep)
            if (pixels[offset] <= colorThreshold ||
    ...
    const result = quantize(pixelArray, colorCount);
...
var quantize = (function() {
    ...
    function getHisto(pixels) {
        ...
        pixels.forEach(function(pixel) {
            ...
            gval = pixel[1] >> rshift;
            bval = pixel[2] >> rshift;
            histo[getColorIndex(rval, gval, bval)] = (histo[index] || 0) + 1;
        });
        return histo;
    }
