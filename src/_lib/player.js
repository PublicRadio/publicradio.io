module.exports = function player(overrides) {
    ...
    return function play(src, opts, cb) {
        ...
        function setSrc(src) {player.src = opts.transformSrc(player.originalSrc = (opts.srcMod instanceof Function ? opts.srcMod(src) : src)) }

        function ended(event) { done(event); }

        function error(event) { done(event); }
